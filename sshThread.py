...
def sshThread(machine, client, flag, num, run) :
	...
	std_list["ssh_stdin{0}".format(num)], std_list["ssh_stdout{0}".format(num)], std_list["ssh_stderr{0}".format(num)] = client.exec_command('cd ' + script_path + ' ; cd ../TestOutput/' + machine + ' ; ./testScript '+str(num)+' '+str(run)+' 1 sghsri')
...
runs = ((input("Number of total runs: "))/4)
...
for num in xrange(0,len(machines)) :
	...
	thread_list["thread{0}".format(num)] = threading.Thread(target=sshThread, args=(machines[num], ssh_list["ssh{0}".format(num)], lock_list["flag{0}".format(num)], num, runs,))
